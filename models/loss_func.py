...
def reconstruction(real_input, gen_input):
    return tf.reduce_mean( tf.abs(real_input-gen_input) )

def loss_generator(prob_fake_is_real):
    return tf.reduce_mean(tf.squared_difference(prob_fake_is_real, 1))

def loss_discriminator_v2(prob_real_is_real, prob_fake_is_real):
...
def loss_discriminator(prob, label):
    return tf.reduce_mean(tf.squared_difference(prob, label))    
...
def reduceXMean( a, b, pids, batch_p, batch_k ):
    same_identity_mask = tf.equal(tf.expand_dims(pids, axis=1),
                                      tf.expand_dims(pids, axis=0))
    mask_XMean_temp = tf.cast(same_identity_mask, tf.float32)
    mask_XMean_temp = tf.multiply( mask_XMean_temp, tf.constant( 1.0/batch_k, 'float32', shape=[] ) )
    mask_XMean = mask_XMean_temp[ ::batch_k,: ]
    diffs = tf.matmul( mask_XMean, a-b  )
    return tf.sqrt(tf.reduce_sum(tf.square(diffs)))/batch_p
    #return mask_XMean, tf.reduce_sum(tf.abs(diffs))/batch_p
